...
import type { AdminActivity, AdminStats, Application, Circle, Feedback, User } from "@/lib/types";

interface AdminData { stats: AdminStats; users: User[]; activities: AdminActivity[]; applications: Application[]; feedbacks: Feedback[]; circles: Circle[]; }
...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const [stats, users, activities, applications, feedbacks, circles] = await Promise.all([
          request<{ stats: AdminStats }>("/admin/stats"), request<{ users: User[] }>("/admin/users?limit=100&offset=0"), request<{ activities: AdminActivity[] }>("/admin/activities?limit=100&offset=0"), request<{ applications: Application[] }>("/admin/applications?limit=100&offset=0"), request<{ feedbacks: Feedback[] }>("/admin/feedbacks?limit=100&offset=0"), request<{ circles: Circle[] }>("/admin/circles?status=pending"),
        ]);
        setData({ stats: stats.stats, users: users.users || [], activities: activities.activities || [], applications: applications.applications || [], feedbacks: feedbacks.feedbacks || [], circles: circles.circles || [] });
      } catch (cause) { setError(cause instanceof ApiError ? cause.message : "管理员数据加载失败"); }
    }
    load();
  }, [router]);
  async function approveCircle(id: string) { try { await request("/admin/circles/" + id + "/approve", { method: "POST" }); setData((p) => p ? { ...p, circles: p.circles.filter((c) => c.id !== id) } : p); } catch { setError("审核操作失败"); } }
  async function rejectCircle(id: string) { try { await request("/admin/circles/" + id + "/reject", { method: "POST" }); setData((p) => p ? { ...p, circles: p.circles.filter((c) => c.id !== id) } : p); } catch { setError("审核操作失败"); } }
  ...
  return <section className="page-shell page-section"><p className="eyebrow">Administration</p><h1 className="page-heading mt-3">平台数据总览</h1><p className="page-subtitle">查看用户、活动、报名、推荐、问卷与反馈的运营概况。</p>{error && <p className="notice-error mt-8">{error}</p>}{!data && !error && <p className="mt-8 text-slate-600">正在加载管理员数据…</p>}{data && <>
    ...
    <AdminSection title="圈子审核">{data.circles.length ? <div className="table-wrap"><table className="data-table"><thead><tr><th>名称</th><th>分类</th><th>创建人</th><th>操作</th></tr></thead><tbody>{data.circles.map((circle) => <tr key={circle.id}><td>{circle.name}</td><td>{circle.category}</td><td>{circle.creator_id || "-"}</td><td><div className="flex gap-2"><button className="button-primary !px-3 !py-1 text-xs" onClick={() => approveCircle(circle.id)}>通过</button><button className="button-danger !px-3 !py-1 text-xs" onClick={() => rejectCircle(circle.id)}>拒绝</button></div></td></tr>)}</tbody></table></div> : <p className="card text-slate-600">暂未有待审核的圈子</p>}</AdminSection>
  </>}</section>;
}
