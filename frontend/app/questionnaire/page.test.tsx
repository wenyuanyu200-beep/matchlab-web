...
describe("QuestionnairePage", () => {
  it("submits array answers and shows the generated profile", async () => {
    ...
    fireEvent.change(screen.getByLabelText("技能标签"), { target: { value: "嵌入式, 焊接" } });
    fireEvent.click(screen.getByRole("button", { name: "生成我的协作画像" }));
    await waitFor(() => expect(postJSON).toHaveBeenCalledWith("/questionnaires", expect.objectContaining({ answers: expect.objectContaining({ interests: ["电赛", "硬件"], skills: ["嵌入式", "焊接"], preferred_activity_types: expect.any(Array) }) })));
