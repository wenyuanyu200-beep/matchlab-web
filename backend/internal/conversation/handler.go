...
func(h *Handler)List(c *gin.Context){rows,err:=h.service.List(c.Request.Context(),current(c));if err!=nil{handle(c,err);return};c.JSON(200,gin.H{"data":gin.H{"conversations":rows}})}
func(h *Handler)Messages(c *gin.Context){id,ok:=param(c);if !ok{return};rows,err:=h.service.Messages(c.Request.Context(),id,current(c));if err!=nil{handle(c,err);return};c.JSON(200,gin.H{"data":gin.H{"messages":rows}})}
func(h *Handler)Post(c *gin.Context){id,ok:=param(c);if !ok{return};var in struct{Content string `json:"content"`};if c.ShouldBindJSON(&in)!=nil{reply(c,400,"invalid_request","invalid request body");return};in.Content=strings.TrimSpace(in.Content);if len([]rune(in.Content))<1||len([]rune(in.Content))>1000{reply(c,400,"invalid_request","content must contain 1 to 1000 characters");return};msg,err:=h.service.Post(c.Request.Context(),id,current(c),in.Content);if err!=nil{handle(c,err);return};c.JSON(201,gin.H{"data":gin.H{"message":msg}})}
func(h *Handler)Read(c *gin.Context){id,ok:=param(c);if !ok{return};if err:=h.service.Read(c.Request.Context(),id,current(c));err!=nil{handle(c,err);return};c.JSON(200,gin.H{"data":gin.H{"read":true}})}
